...
import { TuningCar } from './tuning';
import { TuningContainer, TuningDetailsMain, TuningMenuDetails } from '../styles/tuning';
...
const Tuningdetails = () => {
  const { id } = useParams();
  const TuningDetails = TuningCar.MainDataTuning
  const FilterTuning = TuningDetails.filter((value)=> value.id === parseInt(id))
  console.log(FilterTuning);
  return (
<div>
  <TuningDetailsMain className='Tuning-background'>
    <h1>Aidal</h1>
    <div>
    <Link to={'/add-to-cart'}>
  ...
  <TuningMenuDetails>
    ...
    {/* first container end */}
      <TuningContainer className='container1'>
      ...
      <TuningContainer className='container2'>
      <div>
